...
mod tests {
    ...
    fn connection_bar_selection_change_shows_new_name() {
        // First connection "mia" is highlighted, "ord01" is not.
        let state = three_connection_state();

        let content = render_bar(&state, 120);
        assert!(
            content.contains("mia"),
            "first 'mia' must appear: {content:?}"
        );
        assert!(
            !content.contains("ord01"),
    ...
    fn connection_bar_many_connections_no_overflow_panic() {
        ...
        let state = many_connection_state(20);
    ...
    fn long_selected_name_renders_with_ellipsis() {
        // Make the selected name long to test ellipsis.
        let long_name = "a".repeat(MAX_DISPLAY_NAME_LEN + 1);
        let mut connections = BTreeMap::new();
        connections.insert(long_name.clone(), make_wg_config()); // index 0
        connections.insert("anchor".to_string(), make_wg_config()); // index 1
        let state = AppState::new(AppConfig {
            connections,
            ..AppConfig::default()
        });
        // sel = 0 selects the long-named connection
        let content = render_bar(&state, 120);
        assert!(
            content.contains('…'),
        ...
        assert!(
            ...
            "full long name must not appear: {long_name}"
        );
    }
    ...
    fn name_exactly_at_limit_renders_without_ellipsis() {
        let exact_name = "a".repeat(MAX_DISPLAY_NAME_LEN);
        let mut connections = BTreeMap::new();
        connections.insert(exact_name.clone(), make_wg_config()); // index 0
        connections.insert("anchor".to_string(), make_wg_config()); // index 1
        let state = AppState::new(AppConfig {
            connections,
            ..AppConfig::default()
        });
        // sel = 0 selects exact_name

        let content = render_bar(&state, 120);
        assert!(
            content.contains(exact_name.as_str()),
            "exact name must render without ellipsis: {content:?}"
        );
        assert!(
            !content.contains('…'),
            "exact name must not have ellipsis: {content:?}"
        );
    }
    ...
    fn fifty_connections_no_panic() {
        let state = many_connection_state(50);
        render_bar(&state, 80);
    }
    ...
    fn fifty_connections_selected_name_visible() {
        ...
        // Select index 49 ("c49") so it renders last, after clipping.
        let state = many_connection_state(50); // selected_connection = 49 → "c49"
        let content = render_bar(&state, 120);
        assert!(
    ...
    /// With 20 connections and selected at the first index, the selected name
    /// must be visible.
    #[test]
    fn scrolled_selected_at_start_is_visible() {
        ...
        assert!(
            content.contains("c00"),
            "selected 'c00' must be visible: {content:?}"
        );
    }
    ...
    fn scrolled_selected_in_middle_is_visible() {
        ...
        assert!(
            content.contains("c00"),
            "first 'c00' must be visible: {content:?}"
        );
    }

    /// The `< ` indicator must NOT appear when not scrolled left.
    #[test]
    fn no_left_indicator_when_not_scrolled_left() {
        let state = many_connection_state(20);
        // sel = 0, no scroll left
        let content = render_bar(&state, 80);
        assert!(
            !content.contains('<'),
            "no left-overflow indicator '<' expected: {content:?}"
        );
    }
    ...
    fn scrolled_right_indicator_shown_when_clipped() {
        let state = many_connection_state(20); // selected=19, many entries to the right
        let content = render_bar(&state, 80);
        assert!(
            content.contains('>'),
    ...
    /// When the selected connection is at the first index, the right indicator
    /// must appear (there is more to the right).
    #[test]
    fn right_indicator_when_selected_is_first() {
        let state = many_connection_state(20);

        let content = render_bar(&state, 80);
        assert!(
            content.contains('>'),
            "'>' expected when selected is first entry: {content:?}"
        );
    }
    ...
    fn very_narrow_terminal_selected_visible() {
        ...
        assert!(
            content.contains("c00"),
            "first 'c00' must be visible at 40-col width: {content:?}"
        );
    }
    ...
    fn one_thousand_connections_no_panic() {
        let state = many_connection_state(1000);
        render_bar(&state, 80); // must not panic
    }
    ...
    fn one_thousand_connections_selected_middle_visible() {
        ...
        let content = render_bar(&state, 120);
        assert!(
            content.contains("c0000"),
            "selected 'c0000' must be visible at 1000 connections: {content:?}"
        );
    }
